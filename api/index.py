...
# Dinamikus URL a Web Apphoz
DOMAIN = os.environ.get('VERCEL_URL', 'salesbot1.vercel.app').strip()
WEB_APP_URL = f"https://{DOMAIN}/form.html"
...
def handle_telegram(body_str):
    try:
        ...
        # 1. Web App adat érkezése (Űrlap kitöltés után)
        if "message" in data and "web_app_data" in data["message"]:
            ...
            # Mentés HubSpot-ba (Belső neveket ellenőrizd!)
            props = {
                "szolgaltatas_tipusa": web_data.get("service"),
                "lead_megjegyzes": web_data.get("note")
            }
            if update_hubspot(contact_id, props):
                telegram_request("sendMessage", {"chat_id": data["message"]["chat"]["id"], "text": "✅ Adatok elmentve a HubSpotba!"})
            return

        # 2. Gombnyomás (Kézbe veszem)
        ...
        if cb_data.startswith("claim:"):
            ...
            # HubSpot szerző frissítése
            hub_val = USER_MAPPING.get(t_name)
            if hub_val:
                update_hubspot(contact_id, {"lead_szerzo": hub_val})

            # Telegram válasz (gomb pörgés leállítása)
            telegram_request("answerCallbackQuery", {"callback_query_id": cb_id, "text": "Kijelölve!"})
            
            # Gombok cseréje (Szerző neve + Web App megnyitása)
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Ha van objectId vagy subscriptionType, akkor HubSpot küldte
